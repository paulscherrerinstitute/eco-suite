...
class CameraPositionMonitor(Assembly):
    def __init__(self, pvname, name=None):
        ...
        self.pvname = pvname
        self._append(
            DetectorBsStream, pvname + ":x_fit_mean", cachannel="none", name="xpos"
        )
        self._append(
            DetectorBsStream, pvname + ":y_fit_mean", cachannel="none", name="ypos"
        )
        self._append(
            DetectorBsStream, pvname + ":x_fwhm", cachannel="none", name="xfwhm"
        )
        self._append(
            DetectorBsStream, pvname + ":y_fwhm", cachannel="none", name="yfwhm"
        )
        self._append(
            DetectorBsStream, pvname + ":intensity", cachannel="none", name="intensity"
        )
    ...
    def gui(self):
        self._run_cmd(
            f'caqtdm -macro "NAME={self.pvname},CAMNAME={self.pvname},LED=SLG-LHLP:DUMMY,FLIP=SLG-LHLP:DUMMY" /sf/laser/config/qt/SLG-L-CAM-ALL_ROI_COMP_RF.ui'
        )
...
class CameraPositionMonitorCA(Assembly):
    def __init__(self, pvname, name=None):
        ...
        self._append(
            AdjustablePv, pvname + ":XCALIB", name="xcalib_gradient", is_setting=True
        )
        self._append(
            AdjustablePv, pvname + ":YCALIB", name="ycalib_gradient", is_setting=True
        )
        self._append(
            AdjustablePv, pvname + ":XCALIB-OFFS", name="xcalib_offset", is_setting=True
        )
        self._append(
            AdjustablePv, pvname + ":YCALIB-OFFS", name="ycalib_offset", is_setting=True
        )
