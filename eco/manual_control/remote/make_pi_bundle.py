...
transport, client, mock GUI, constants, pi_client entry point). The
PC/eco-side modules (box, navigator, jog, snapshot, server, demo) are
deliberately excluded. All imports inside the bundle are package-relative,
so it works under the top-level name `manual_control` unchanged.
"""
...
def main():
    ...
    print("Copy that manual_control/ folder to the Pi, then run e.g.:")
    print("  python3 -m manual_control.remote.pi_client --serial /dev/rfcomm0")
